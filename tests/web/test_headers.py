...
def test_explicit(webapp):
    f = urlopen("{:s}/foo".format(webapp.server.http.base))
    s = f.read()
    assert s == b"Hello World!"
...
def test_static(webapp):
    f = urlopen("{:s}/static/test.css".format(webapp.server.http.base))
    s = f.read()
    assert s == b"body { }\n"
...
def test_empty(webapp):
    f = urlopen("{:s}/empty".format(webapp.server.http.base))
    s = f.read()
    assert s == b""

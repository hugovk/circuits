...
def PING(server):
    return Message("PING", ":{}".format(server))


def PONG(server, text):
    return Message("PONG", server, ":{}".format(text))
...
def RPL_WELCOME(network):
    return _M("001", "Welcome to the {} IRC Network".format(network))


def RPL_YOURHOST(host, version):
    return _M("002", "Your host is {} running {}".format(host, version))


def RPL_CREATED(date):
    return _M("003", "This server was created {}".format(date))
...
def RPL_LUSEROP(noperators):
    return _M("252", "{}".format(noperators), "operator(s) online")


def RPL_LUSERUNKOWN(nunknown):
    return _M("253", "{}".format(nunknown), "unknown connection(s)")


def RPL_LUSERCHANNELS(nchannels):
    return _M("254", "{}".format(nchannels), "channels formed")


def RPL_LUSERME(nclients, nservers):
    return _M("255", "I have {} clients and {} servers".format(nclients, nservers))


def RPL_AWAY(nick, message):
    return _M("301", nick, ":{}".format(message))
...
def RPL_WHOISUSER(nick, user, host, realname):
    return _M("311", nick, user, host, "*", ":{}".format(realname))
...
def RPL_LIST(channel, nvisible, topic):
    return _M("322", channel, "{}".format(nvisible), topic)
...
def RPL_TOPICWHO(channel, setter, timestamp):
    return _M("333", channel, setter, "{}".format(timestamp))


def RPL_INVITING(channel, nick):
    return _M("341", "{} {}".format(channel, nick))


def RPL_SUMMONING(user):
    return _M("342", "{} :Summoning user to IRC".format(user))


def RPL_INVITELIST(channel, invitemask):
    return _M("346", "{} {}".format(channel, invitemask))


def RPL_ENDOFINVITELIST(channel):
    return _M("347", "{} :End of channel invite list".format(channel))
...
def RPL_WHOREPLY(channel, user, host, server, nick, status, hops, name):
    return _M(
        "352", channel, user, host, server, nick, status, ":{} {}".format(hops, name)
    )
...
def RPL_MOTD(text):
    return _M("372", "- {}".format(text))


def RPL_MOTDSTART(server):
    return _M("375", "- {} Message of the day -".format(server))
